...
With `announcements.serverPreparedAudio = true`, the server builds announcement and alarm PCM once, packs it into Rednet `audio_stream` chunks, and broadcasts those prepared chunks to kiosks/controllers. Clients then only unpack the signed 8-bit samples and feed their attached speakers; they do not load WAVs, resample audio, build voice lines, or synthesize alarm buffers unless `announcements.clientAudioSynthesis = true` is explicitly enabled. Tune `announcements.remoteAudioChunkSamples` and `announcements.remoteAudioLeadSeconds` for network size and playback lead time.
