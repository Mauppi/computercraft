...
Employee account data, personal notes, social posts, and direct messages are not stored in the GitHub config. They remain in the files configured by `employees.accountsFile` and `employees.socialFile`, defaulting to `security_accounts.lua` and `security_social.lua`; the auto-updater does not download or overwrite those files. The generic manifest downloader also skips protected local-state filenames such as `security_config.lua`, `security_local_data.lua`, `security_accounts.lua`, `security_social.lua`, and `security_audit.log`.
