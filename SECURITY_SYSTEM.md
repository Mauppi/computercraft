...
If a writable disk drive is attached, `startup_auto_update.lua` installs downloaded app files and WAV assets under `disk/security_system` or the largest available `disk*/security_system` mount instead of filling the computer's local storage. The computer-local `security_config.lua` remains on the computer, so server/kiosk mode and local identity do not move unexpectedly. WAV playback checks normal config paths first, then the updater's disk install root.
