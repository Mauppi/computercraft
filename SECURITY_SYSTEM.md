...
`startup_auto_update.lua` fetches `security_system_manifest.lua` from `https://raw.githubusercontent.com/Mauppi/computercraft/master/security_system_manifest.lua`, then downloads every listed app file from the manifest's `baseUrl`. The required files are `security_system_defaults.lua`, `security_system_rednet.lua`, `security_system_notifications.lua`, `security_system_announcements.lua`, `security_system_app.lua`, and `security_system.lua`; the manifest can also keep startup scripts, examples, docs, and WAV assets synced. The updater also downloads `startup_auto_update.lua` from GitHub and refreshes the currently running startup script, so computers that copied it to `startup.lua` can self-update on the next run. Its default after-update behavior is `run` for server mode and `reboot` for kiosk mode. HTTP must be enabled in the CC:Tweaked server config.

If a writable disk drive is attached and has enough free space, `startup_auto_update.lua` installs downloaded app files and WAV assets under `disk/security_system` or the largest available `disk*/security_system` mount instead of filling the computer's local storage. Normal small CC floppy disks may be too small for the full app, so the updater falls back to computer storage when the largest disk has less than the required free-space threshold. The runtime `security_config.lua` remains computer-local, but it is refreshed from GitHub on every auto-update run. WAV playback checks normal config paths first, then the updater's disk install root.

On every server auto-update run, `startup_auto_update.lua` downloads `security_config.lua` from GitHub using the manifest `baseUrl`, then merges computer-local facility data before writing the local `security_config.lua`. The GitHub config remains the base template, while `security_local_data.lua` preserves stateful setup sections such as `doors`, `readers`, `credentials`, `sensors`, `emergencyButtons`, `generators`, monitor device mappings, kiosk controller mode, and local account/social file paths. If the updater cannot download and validate a server-mode config, it refuses to start instead of letting the app generate built-in defaults. The app has the same missing-config safeguard for manual starts.

For kiosk, controller, and door-controller auto-update modes, `startup_auto_update.lua` downloads `security_kiosk_config.example.lua` from GitHub, merges computer-local data into the runtime `security_config.lua`, and refreshes the local installed `security_kiosk_config.example.lua` with the clean GitHub template. The updater no longer asks the main security server for startup config; the GitHub kiosk config plus `security_local_data.lua` are authoritative. After config refresh, the updater scans `announcements`, `notifications`, and `alarm` for every `.wav` path in voice lines, jingles, notification sounds, and alarm sounds, downloads those files in binary mode from the manifest `baseUrl`, `announcements.assetBaseUrl`, `notifications.assetBaseUrl`, or `alarm.assetBaseUrl`, and validates that each downloaded clip is a WAV file. A fresh kiosk still needs kiosk mode selected before its first config exists; set `MODE_OVERRIDE = "kiosk"` in `startup_auto_update.lua`, run `startup_auto_update kiosk` once, create `security_mode.txt` or `.security_mode` containing `kiosk`, or label the computer with `kiosk`.

Employee account data, personal notes, social posts, direct messages, and saved alarm state are not stored in the GitHub config. They remain in local files: employee data uses the files configured by `employees.accountsFile` and `employees.socialFile`, defaulting to `security_accounts.lua` and `security_social.lua`, while alarm state uses `alarm.stateFile`, defaulting to `security_alarm_state.lua`. The auto-updater does not download or overwrite those files. The generic manifest downloader also skips protected local-state filenames such as `security_config.lua`, `security_local_data.lua`, `security_alarm_state.lua`, `security_accounts.lua`, `security_social.lua`, and `security_audit.log`.
...
With `announcements.serverPreparedAudio = true`, the server builds announcement and alarm PCM once, splits it into compact Rednet `audio_stream` signed-byte chunks, and broadcasts those prepared chunks to kiosks/controllers. Clients then only assemble the signed 8-bit samples and feed their attached speakers; they do not load WAVs, resample audio, build voice lines, or synthesize alarm buffers unless `announcements.clientAudioSynthesis = true` is explicitly enabled. Tune `announcements.remoteAudioChunkSamples`, `announcements.remoteAudioYieldChunks`, `announcements.remoteAudioYieldSeconds`, and `announcements.remoteAudioLeadSeconds` for network size, sender pacing, and playback lead time. Defaults favor fewer, paced packets: `12000` samples per packet, one packet per `0.05` seconds, and `2.5` seconds of lead time.
