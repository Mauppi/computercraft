...
On every server auto-update run, `startup_auto_update.lua` downloads `security_config.lua` from GitHub using the manifest `baseUrl` and overwrites the local `security_config.lua` before starting the app. This makes the repository config authoritative; setup wizard changes made in-game should be copied back to the GitHub config or they will be replaced on the next update. If the updater cannot download and validate a server-mode config, it refuses to start instead of letting the app generate built-in defaults. The app has the same missing-config safeguard for manual starts.

For kiosk, controller, and door-controller auto-update modes, `startup_auto_update.lua` downloads `security_kiosk_config.example.lua` from GitHub, writes that downloaded file to the runtime `security_config.lua`, and refreshes the local installed `security_kiosk_config.example.lua`. The updater no longer asks the main security server for startup config; the GitHub kiosk config is authoritative. After config refresh, the updater scans `announcements`, `notifications`, and `alarm` for every `.wav` path in voice lines, jingles, notification sounds, and alarm sounds, downloads those files in binary mode from the manifest `baseUrl`, `announcements.assetBaseUrl`, `notifications.assetBaseUrl`, or `alarm.assetBaseUrl`, and validates that each downloaded clip is a WAV file. A fresh kiosk still needs kiosk mode selected before its first config exists; set `MODE_OVERRIDE = "kiosk"` in `startup_auto_update.lua`, run `startup_auto_update kiosk` once, create `security_mode.txt` or `.security_mode` containing `kiosk`, or label the computer with `kiosk`.
...
An authorized kiosk can also make itself a permanent door controller from `Facility setup` -> `This kiosk door-controller mode`. This keeps the employee kiosk UI running, saves `kiosk.controller.enabled = true` in the kiosk's local `security_config.lua`, answers server endpoint read/write/scan requests, and forwards local RFID/NFC/card scans as controller reader sources. The next auto-update will replace the local kiosk config from `security_kiosk_config.example.lua`, so keep permanent controller defaults in the GitHub kiosk config or select controller mode with `security_mode.txt`, `.security_mode`, startup args, or `MODE_OVERRIDE`.
